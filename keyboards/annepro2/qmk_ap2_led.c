...
void annepro2LedUpdate(uint8_t row, uint8_t col) {
  ...
  sdPut(&SD0, row);
  sdPut(&SD0, col);
  sdWrite(&SD0, (uint8_t *)&annepro2LedMatrix[row * MATRIX_COLS + col],
          sizeof(uint16_t));
}
...
void annepro2LedUpdateRow(uint8_t row) {
  ...
  sdPut(&SD0, row);
  sdWrite(&SD0, (uint8_t *)&annepro2LedMatrix[row * MATRIX_COLS],
          sizeof(uint16_t) * MATRIX_COLS);
}
...
void annepro2LedNextIntensity() { sdPut(&SD0, CMD_LED_NEXT_INTENSITY); }

void annepro2LedNextAnimationSpeed() {
  sdPut(&SD0, CMD_LED_NEXT_ANIMATION_SPEED);
}

void annepro2LedPrevProfile() { sdPut(&SD0, CMD_LED_PREV_PROFILE); }
