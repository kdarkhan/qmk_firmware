...
bool spi_eeprom_start(void) {
  return spi_start(EXTERNAL_EEPROM_SPI_SLAVE_SELECT_PIN,
                   EXTERNAL_EEPROM_SPI_LSBFIRST, EXTERNAL_EEPROM_SPI_MODE,
                   EXTERNAL_EEPROM_SPI_CLOCK_DIVISOR);
}
...
void eeprom_read(void *buf, uint32_t addr, size_t len) {
  ...
  spi_status_t response =
      spi_eeprom_wait_while_busy(EXTERNAL_EEPROM_SPI_TIMEOUT);
  spi_stop();
  if (response == SPI_STATUS_TIMEOUT) {
    dprint("SPI timeout for WIP check\n");
    spi_stop();
