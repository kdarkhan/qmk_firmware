...
#define PAFIO_L(PORT, LINE, AF)                                                \
  (((PAL_PORT(LINE) == PORT) && (PAL_PAD(LINE) < 8))                           \
       ? (AF << (PAL_PAD(LINE) << 2))                                          \
       : 0)
#define PAFIO_H(PORT, LINE, AF)                                                \
  (((PAL_PORT(LINE) == PORT) && (PAL_PAD(LINE) >= 8))                          \
       ? (AF << ((PAL_PAD(LINE) - 8) << 2))                                    \
       : 0)
#define PAFIO(PORT, N, LINE, AF)                                               \
  ((N) ? PAFIO_H(PORT, LINE, AF) : PAFIO_L(PORT, LINE, AF))

#define OUT_BITS(PORT)                                                         \
  (PBIT(PORT, LINE_ROW1) | PBIT(PORT, LINE_ROW2) | PBIT(PORT, LINE_ROW3) |     \
   PBIT(PORT, LINE_ROW4) | PBIT(PORT, LINE_ROW5) | 0)

#define IN_BITS(PORT)                                                          \
  (PBIT(PORT, LINE_COL1) | PBIT(PORT, LINE_COL2) | PBIT(PORT, LINE_COL3) |     \
   PBIT(PORT, LINE_COL4) | PBIT(PORT, LINE_COL5) | PBIT(PORT, LINE_COL6) |     \
   PBIT(PORT, LINE_COL7) | PBIT(PORT, LINE_COL8) | PBIT(PORT, LINE_COL9) |     \
   PBIT(PORT, LINE_COL10) | PBIT(PORT, LINE_COL11) | PBIT(PORT, LINE_COL12) |  \
   PBIT(PORT, LINE_COL13) | PBIT(PORT, LINE_COL14) | 0)
...
#define AF_BITS(PORT, N)                                                       \
  (PAFIO(PORT, N, LINE_UART_RX, AFIO_USART) |                                  \
   ...
   PAFIO(PORT, N, LINE_COL14, AFIO_GPIO) | 0)
...
const ioline_t col_list[MATRIX_COLS] = {
    LINE_COL1,  LINE_COL2,  LINE_COL3,  LINE_COL4,  LINE_COL5,
    LINE_COL6,  LINE_COL7,  LINE_COL8,  LINE_COL9,  LINE_COL10,
    LINE_COL11, LINE_COL12, LINE_COL13, LINE_COL14,
};
